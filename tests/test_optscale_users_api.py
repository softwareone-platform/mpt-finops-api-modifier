...
class TestOptscaleAPI:
    ...
    async def test_create_duplicate_user(self, optscale_api, mock_post):
        ...
        ), "Expected error response for duplicate user creation"

    async def test_create_user_exception_handling(
        self, optscale_api, mock_post, caplog
    ):
        ...
        with caplog.at_level(logging.ERROR):
            ...
            with pytest.raises(
                UserCreationError,
                match="An unexpected error occurred " "while creating the user.",
            ):
    ...
    @pytest.mark.parametrize(
        ("user_id", "expected_status", "expected_reason"),
        [  # noqa: PT006
            (USER_ID, 200, None),
            (
                INVALID_USER_ID,
                404,
                "User with id f0bd0c4a-7c55-45b7-8b58-27740e38789k not found",
            ),
        ],
    )
    async def test_get_user_by_id(
        self, optscale_api, mock_get, user_id, expected_status, expected_reason
    ):
        ...
        result = await optscale_api.get_user_by_id(
        ...
        assert result == mock_response, (
            f"Expected response for user_id={user_id} " f"with status {expected_status}"
        )
    ...
    async def test_get_user_with_invalid_admin_api_key(self, optscale_api, mock_get):
        ...
        result = await optscale_api.get_user_by_id(
            user_id=USER_ID, admin_api_key="invalid_key"
        )
        ...
        assert (
            result == mock_response
        ), "Expected 403 response for invalid admin API key"
